...
sep.panel <- function(data) {
  ...
  RI.ktkg <- RI[which(RI$type %in% "KTKG"), -which(names(RI) %in% "type")]
  RI.ntkg <- RI[which(RI$type %in% "NTKG"), -which(names(RI) %in% "type")]
  RI.ntng <- RI[which(RI$type %in% "NTNG"), -which(names(RI) %in% "type")]
  IB.ktkg <- IB[which(IB$type %in% "KTKG"), -which(names(IB) %in% "type")] 
  IB.ntkg <- IB[which(IB$type %in% "NTKG"), -which(names(IB) %in% "type")]
  IB.ntng <- IB[which(IB$type %in% "NTNG"), -which(names(IB) %in% "type")]
  ...
  return(list(RI_DF = RI, InbredDF = IB, KN_Results =knrslt, NTKG_Results = ntkgrslt, NTNG_Results = ntngrslt))
}